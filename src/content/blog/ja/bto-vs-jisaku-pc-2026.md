...
5000番台が安定流通したことで、「BTO の納期 1ヶ月待ち」が解消し、急ぎ需要での BTO 優位がやや薄れました。とはいえ「組立工数ゼロ」「サポート窓口一括」というBTOの本質的なメリットは変わりません。
...
「BTO か自作か」は宗教論争のように扱われがちですが、実質はライフスタイルとリスク許容度の選択です。同じ予算で同じ用途を満たせるなら、自分が「楽な側」を選ぶのが正しい答えです。組立そのものを楽しめないなら、3万円差は休日 1日と引き換えるのに見合いません。
